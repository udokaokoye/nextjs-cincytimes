...
export function GetCategories() {
  const { data, error } = useSWR(`http://192.168.1.158/cincinnatitimes/get/get_category_names.php`, fetcher )
  return {
  categories: data,
  isLoad: !error && !data,
  isError: error,
...
export function GetComments(postId, order = 'DESC') {
  const { data, error } = useSWR(`http://192.168.1.158/cincinnatitimes/get/get_comments.php?post_id=${postId}&order=${encodeURI(order)}`, fetcher )
  // console.log(data);
  return {
  comments: data,
  isLoad: !error && !data,
...
export function GetAdminEditPosts(postid, published) {
  const { data, error } = useSWR(`http://192.168.1.158/cincinnatitimes/get/get_admin_post.php?${published ? `published=${published}` : ''}${postid ? `&post_id=${postid}` : ''}`, fetcher )
  // console.log(data)
  return {
  adminPosts: data,
  isLoad: !error && !data,
